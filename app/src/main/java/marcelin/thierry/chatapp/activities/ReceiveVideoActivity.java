...
public class ReceiveVideoActivity extends AppCompatActivity {
    ...
    private ImageView mRejectCall;

    private LinearLayout mCall, mCallBar, callLayout;

    private FrameLayout mRemoteVideoViewContainer, mLocalVideoviewContainer;
...
// to wake up screen
        ...
        if (pm != null) {
            wakeLock = pm.newWakeLock((PowerManager.SCREEN_BRIGHT_WAKE_LOCK
                    ...
                    "marcelin.thierry.chatapp:mywakelogtack");
        }
        if (wakeLock != null) {
...
// to release screen lock
        ...
        mCallBar = findViewById(R.id.callBar);
        callLayout = findViewById(R.id.callLayout);

        mRemoteVideoViewContainer = findViewById(R.id.remote_video_view_container);
        mLocalVideoviewContainer = findViewById(R.id.local_video_view_container);

        String nameStored = Users.getLocalContactList().get(mUserPhone);
        nameStored = nameStored != null && nameStored.length() > 0 ? nameStored :
                mUserPhone;

        if (nameStored.length() > 25) {
            nameStored = nameStored
                    .substring(0, 21) + "...";
        }
        mCaller.setText(nameStored);

        // Ring phone
        mRingTone = MediaPlayer.create(this, R.raw.ic_ringing);
        ...
        mAcceptCall = findViewById(R.id.acceptCall);
        mRejectCall = findViewById(R.id.rejectCall);

        mAcceptCall.setOnClickListener(view -> {
            ...
            mRemoteVideoViewContainer.setVisibility(View.VISIBLE);
            mLocalVideoviewContainer.setVisibility(View.VISIBLE);
            callLayout.setVisibility(View.VISIBLE);

        });

        mRejectCall.setOnClickListener(view -> {
            joinChannel();
            finish();
        });
